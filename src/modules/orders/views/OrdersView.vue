            ...
            <div v-else class="orders-page__list">
                <div v-for="order in orders" :key="order.id" class="order-card-view">
                    ...
                    <div class="order-card-view__footer">
                        <BaseButton variant="outline" size="sm" @click="showOrderDetail(order)">Ver Detalle Completo
                        </BaseButton>
                        <BaseButton v-if="order.status === 'pending'" variant="text" size="sm"
                            @click="handleCancelRequest">Cancelar Pedido</BaseButton>
                    </div>
                </div>
                ...
                <nav v-if="totalPages > 1" class="pagination-orders">
                    ...
                    <button v-for="page in totalPages" :key="page" class="pagination-orders__btn"
                        :class="{ 'pagination-orders__btn--active': page === currentPage }" @click="changePage(page)">
                        {{ page }}
                    </button>
                    ...
                    <div class="modal-orders__content" @click.stop>
                        ...
                        <div class="modal-orders__body">
                                ...
                                <div class="items-list-detail-orders">
                                    <div v-for="item in selectedOrder.items" :key="item.id" class="item-detail-orders">
                                        ...
                                        <div class="item-detail-orders__info">
                                            ...
                                            <span class="item-detail-orders__meta">{{ item.quantity }} x ${{
                                                formatPrice(item.price) }}</span>
                                        </div>
                                        <span class="item-detail-orders__subtotal">${{ formatPrice(item.price *
                                            item.quantity) }}</span>
                                    </div>
                                </div>
                            </section>
                            ...
                            <div class="grid-details-orders">
                                <section class="section-modal-orders">
                                    ...
                                    <div class="section-modal-orders__content">
                                        ...
                                        <p><strong>Ciudad:</strong> {{ selectedOrder.city }}, {{ selectedOrder.region }}
                                        </p>
                                    </div>
                                </section>
                            ...
                            <div class="total-display-orders">
                                ...
                                <span class="total-display-orders__amount">${{ formatPrice(selectedOrder.totalPrice)
                                    }}</span>
                            </div>
                        </div>
                    </div>
